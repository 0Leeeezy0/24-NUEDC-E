...
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    {
        Count_X++;
        gpio_toggle_level(PWM_X_Left);
        gpio_toggle_level(PWM_X_Right);
        if(Count_X>=(Step_X*2))//基于一个脉冲等于上升沿加下降沿得出与步进值的两倍关系
        {
            Count_X=0;
            Step_X_absolute+=Step_OnceX_relative;//更新x轴的绝对坐标
            MotorX_Flag=0;
        }
    }
    if(MotorY_Flag==1)
    {
        Count_Y++;
        gpio_toggle_level(PWM_Y);
        if(Count_Y>=(Step_Y*2))//基于一个脉冲等于上升沿加下降沿得出与步进值的两倍关系
        {
            Step_Y_absolute+=Step_OnceY_relative;//更新Y轴的绝对坐标
            Count_Y=0;
            MotorY_Flag=0;
        }
    }
    ...
    {
        Count_Z++;
        gpio_toggle_level(PWM_Z);
        if(Count_Z>=(Step_Z*2))//基于一个脉冲等于上升沿加下降沿得出与步进值的两倍关系
        {
            Step_Z_absolute+=Step_OnceZ_relative;//更新Y轴的绝对坐标
            Count_Z=0;
            MotorZ_Flag=0;
        }
    }
...
{
    static int Key_Scan_Count=0;

    interrupt_global_enable(0);                     // 开启中断嵌套
    pit_clear_flag(CCU60_CH1);

    Key_Scan_Count++;
    if(Key_Scan_Count>=20)
    {
        Key_Scan_Count=0;
        Key_Scan();
    }
